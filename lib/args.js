...
function showHelp(platform) {
  console.log(`
  ${"\x1b[1m"}ForgeZero${"\x1b[0m"} - Modular & Intelligent Assembly Builder
    No dependecies and others bullshit. Just install node in ur pc;)
  ...
  ${"\x1b[1m"}OPTIONS:${"\x1b[0m"}
    ...
    -a, --assembler // --asm <asm>     nasm or gas (default: ${DEFAULTS.assembler})
    -f, --format <fmt>        Object format for NASM/FASM (default: ${DEFAULTS.format})
    ...
    --debug                   Generate debug symbols (DWARF)
  ...
  ${"\x1b[1m"}EXAMPLES:${"\x1b[0m"}
    ...
    $ node index.js --assembler fasm --debug --output test test.fasm
...
function parseArgs(argv, logger) {
  ...
  if (options.debug && !options.linkFlags.includes('-g')) options.linkFlags += ' -g';
