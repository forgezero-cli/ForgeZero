...
func ProcessGloria(path string, outBin string) error {
	if filepath.Ext(path) != ".glo" {
		return Errorf("Gloria file must have .glo extension!")
	}
	
	srcBytes, err := os.ReadFile(path)
	if err != nil {
		return Errorf("error reading Gloria file: %v", err)
	}
	
	machineCode, err := gloria.Emit(string(srcBytes))
	if err != nil {
		return Errorf("Gloria compiler error: %v", err)
	}
	...
	if err != nil {
		return Errorf("error writing output binary: %v", err)
	}
