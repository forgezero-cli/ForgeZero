...
func (m *Module) Lookup(sym string) (unsafe.Pointer, error) {
    if m == nil || m.h == nil { return nil, errors.New("not loaded") }
    cs := C.CString(sym)
    defer C.free(unsafe.Pointer(cs))
    p := C.symbol(m.h, cs)
    if p == nil { return nil, errors.New(C.GoString(C.dlerr())) }
    return unsafe.Pointer(p), nil
}

func (m *Module) Close() error {
    if m == nil || m.h == nil { return nil }
    r := C.close_lib(m.h)
    m.h = nil
    if r != 0 { return errors.New("dlclose failed") }
    return nil
}
...
func (m *Module) CallModuleEntry(moduleSym string, ctx unsafe.Pointer) error {
    if m == nil || m.h == nil { return errors.New("not loaded") }
    cs := C.CString(moduleSym)
    defer C.free(unsafe.Pointer(cs))
    r := C.call_module_entry_by_modsym(m.h, cs, ctx)
    if r != 0 { return errors.New("module entry call failed") }
    return nil
}
